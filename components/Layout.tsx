...
export const Layout: React.FC<LayoutProps> = ({ 
  children, activeTab, onTabChange, currentUser, onLogout, isDarkMode = false, onToggleDarkMode 
}) => {
      ...
      >
        <div className={`flex flex-col h-full ${isSidebarCollapsed ? 'p-4' : 'p-8'}`}>
          <div className="mb-10 flex items-center justify-between">
            <div className="flex items-center space-x-4 cursor-pointer overflow-hidden" onClick={() => onTabChange('home')}>
          ...
          <div className={`mt-4 space-y-3 pt-6 border-t overflow-hidden ${isDarkMode ? 'border-white/5' : 'border-gray-50'}`}>
            ...
            {currentUser ? (
              <button onClick={() => onTabChange('my-profile')} className={`flex items-center w-full text-left rounded-2xl transition-all border focus:outline-none ${isSidebarCollapsed ? 'p-2 justify-center' : 'p-3 space-x-3'} ${activeTab === 'my-profile' ? (isDarkMode ? 'bg-white text-black border-white shadow-xl' : 'bg-ffn-black text-white border-ffn-black shadow-xl') : (isDarkMode ? 'bg-white/5 border-transparent text-white' : 'bg-gray-50 border-transparent text-ffn-black')}`}>
                <div className="w-8 h-8 rounded-xl overflow-hidden border-2 border-white/10 shadow-sm flex-none"><img src={currentUser.avatarUrl} className="w-full h-full object-cover" alt="" /></div>
                {!isSidebarCollapsed && <div className="flex flex-col leading-tight overflow-hidden"><span className={`text-[8px] font-bold uppercase tracking-widest truncate ${isDarkMode && activeTab !== 'my-profile' ? 'text-white' : ''}`}>{currentUser.displayName}</span><span className={`text-[6px] uppercase font-black ${isDarkMode && activeTab !== 'my-profile' ? 'text-white/40' : 'text-gray-400'}`}>Identity Ready</span></div>}
              </button>
            ) : (
              <button 
              ...
              >
                <LogIn className={`w-4 h-4 relative z-10 ${isDarkMode ? 'text-white' : 'text-white'}`} />
                {!isSidebarCollapsed && (
                  <div className="flex flex-col leading-tight relative z-10">
                    <span className={`text-[9px] font-bold uppercase tracking-[0.1em] ${isDarkMode ? 'text-white' : 'text-white'}`}>Sign In</span>
                    <span className={`text-[6px] uppercase font-black ${isDarkMode ? 'text-white/40' : 'text-white/40'}`}>Entry Protocol</span>
                  </div>
                )}
              </button>
            )}
